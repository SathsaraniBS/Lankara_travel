      ...
      <div className="max-w-6xl mx-auto space-y-10">
        ...
        <div className="bg-[#121614] border border-zinc-800/80 rounded-2xl p-4 sm:p-6 flex flex-col sm:flex-row items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-2 overflow-x-auto w-full sm:w-auto pb-2 sm:pb-0">
            <Filter size={16} className="text-zinc-500 hidden sm:block mr-2" />
            {categories.map((cat) => (
              <button
                key={cat}
                onClick={() => setSelectedCategory(cat)}
                className={`text-xs px-4 py-2 rounded-xl transition whitespace-nowrap font-medium ${
            ...
            >
              <div>
                ...
                <div className="relative h-56 w-full overflow-hidden">
                  ...
                  />
                  <div className="absolute top-3 left-3 bg-black/60 backdrop-blur-md text-emerald-400 text-[10px] font-bold px-3 py-1 rounded-full border border-emerald-500/20">
                    {pkg.category}
                  </div>
                  {pkg.featured && (
                    <div className="absolute top-3 right-3 bg-amber-500 text-slate-950 text-[10px] font-extrabold px-2.5 py-1 rounded-full">
                      POPULAR
                    </div>
                  )}
                </div>
